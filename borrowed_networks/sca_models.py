...
class Channel_wise_attention(nn.Module):
    ...
    def forward(self, feature_map, decoder_hidden):
        ...
        # print(decoder_hidden.shape,self.W_hc.shape)
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size,encoder_out_shape=[8,2048,8,8], K=512,encoder_dim=2048, dropout=0.5):
        """
        ...
        self.decode_step = nn.LSTMCell(encoder_dim+embed_dim, decoder_dim, bias=True)  # decoding LSTMCell
    ...
    def init_hidden_state(self, encoder_out):
        ...
        # print(mean_encoder_out.shape,self.encoder_dim,self.decoder_dim)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        # print(caption_lengths.cpu().numpy()-1)

        caption_lengths=caption_lengths.cpu().numpy()-1
        decode_lengths = torch.tensor((caption_lengths).tolist()).cuda()
        ...
        predictions = torch.zeros(batch_size, max(decode_lengths).cpu().numpy(), vocab_size).to(device)#需要更改形状？
        ...
        for t in range(max(decode_lengths)):
            ...
            # print(preds.shape)  # (batch_size_t, vocab_size)
